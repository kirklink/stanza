...
import 'sqlite_ddl.dart';
import 'sqlite_introspector.dart';
import 'sqlite_migration_runner.dart';

/// Orchestrates schema management for SQLite: diff, generate, and apply migrations.
///
...
class SqliteSchemaManager {
  ...
  late final _introspector = SqliteIntrospector(_db);
  late final _runner =
      SqliteMigrationRunner(_db, migrationsDir: _migrationsDir);
  ...
  /// Generates a SQLite migration file from the current diff.
  ///
  ...
  Future<String?> generate() async {
    ...
    final content = SqliteDdl.generateMigration(ops);
  ...
  Future<List<SqliteMigrationStatus>> status() => _runner.status();
